...
import copy
import time
import sentinel.utils.mocap_client.data_descriptions as DataDescriptions
import sentinel.utils.mocap_client.mocap_data as MoCapData
...
class NatNetClient:
    ...
    def __init__(self):
        ...
        if IN_DOMESTIC_SUITE:
            self.server_ip_address = "172.24.69.162"  # Domestic Suite
        else:
            self.server_ip_address = "172.24.68.19"  # Dog Room
    ...
    def __unpack_legacy_other_markers(self, data, packet_size, major, minor):
        ...
        if other_marker_count > 0:
            ...
            for j in range(0, other_marker_count):
                ...
                offset += 12
                trace_mf(
                    "\tMarker %3.1d : [x=%3.2f,y=%3.2f,z=%3.2f]"
                    % (j, pos[0], pos[1], pos[2])
                )
    ...
    def __unpack_labeled_marker_data(self, data, packet_size, major, minor):
        ...
        if (major == 2 and minor > 3) or major > 2:
            ...
            for lm_num in range(0, labeled_marker_count):
                ...
                offset += 4
                trace_mf(
                    "%3.1d ID     : [MarkerID: %3.1d] [ModelID: %3.1d]"
    ...
    def __unpack_server_info(self, data, packet_size, major, minor):
        ...
        if (self.__nat_net_requested_version[0] == 0) and (
            ...
            print(
                "resetting requested version to %d %d %d %d from %d %d %d %d"
                % (
